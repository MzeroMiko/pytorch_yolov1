...
# one way to modify given model
class resnetYOLO(models.resnet.ResNet):
    def __init__(self, block, layers, **kwargs):
        super(resnetYOLO, self).__init__(block, layers, **kwargs)
        del self.avgpool
        del self.fc
    ...
    def forward(self, x):
        ...
        x = self.layer5(x) # batch_size*256*14*14
        x = self.conv_end(x) # batch_size*30*14*14
        x = self.bn_end(x) # batch_size*30*14*14
        x = x.sigmoid().permute(0,2,3,1) # batch_size*14*14*30
        return x
...
def resnet50(pretrained=False, model_dict='', **kwargs):
    model = resnetYOLO(models.resnet.Bottleneck, [3, 4, 6, 3], **kwargs)
